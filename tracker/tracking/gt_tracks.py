...
class GtTrackLoader:
    ...
    EXCLUDE_TYPES = {"DontCare", "bicycle_rack", "ride_uncertain", "Pedestrian",
                     "rider"}
    # `rider` NO es una clase más: VoD anota cada ciclista dos veces, con una
    # caja `Cyclist` (persona + bicicleta) y otra `rider` (solo la persona),
    # prácticamente superpuestas — el 98.7% de los Cyclist del split de
    # validación tienen un `rider` a menos de 1 m. Contar las dos duplica el
    # objeto en el GT, en las detecciones y en las métricas, y además genera
    # pares de tracks solapados. Por eso se descarta SIEMPRE, incluso cuando se
    # pide "todas las clases" (que solo pretende no excluir peatón y demás).
    SIEMPRE_EXCLUIDAS = {"rider"}
    ...
    def __init__(self, dataset_path, keep_types=None, min_radar_points=0,
                 fov_only=False, moving_only=False, exclude_types=None):
        ...
        `exclude_types`: reemplaza la lista de clases descartadas. Por defecto se
        usa `EXCLUDE_TYPES` (que saca peatón). Pasar `{"DontCare"}` reproduce el
        criterio de RaTrack, que solo descarta la caja placeholder de KITTI.
        """
    ...
    def load_frame(self, frame_id):
        ...
        for i, line in enumerate(lines):
            ...
            if lab["type"] in self.EXCLUDE_TYPES or lab["type"] in self.SIEMPRE_EXCLUIDAS:
                continue
